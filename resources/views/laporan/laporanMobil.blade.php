...
<body>
    ...
    <p>Pendapatan dari {{ $count }} transaksi.</p>
    <hr style="border:3px solid #f1f1f1">

    <div class="table-responsive col-lg-12 py-2">
        <h3 id="transaksis-title">{{ $tahun }}/{{ strtoupper(DateTime::createFromFormat('!m', $bulan)->format('F')) }} LAPORAN MOBIL </h3>
            ...
            <tbody>
                @foreach($mobils as $mobil)
                    <tr>
                        ...
                        <td>{{ $mobil->pelat_nomor }}</td>
                        <td>{{ $mobil->jumlah }}</td>
                        <td>Rp {{$mobil->subtotal}}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
